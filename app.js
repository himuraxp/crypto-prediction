  ...
  if (messageText) {
    ...
    if (messageText.match(hashtag)) {
      ...
      recastClient.analyseText(messageText).then(function(response) {
        ...
        } else if (intent == 'wru-1' || intent == 'wru') {
 ...
 *
 */
function sendInfoMessage(recipientId, cInfo) {
  var messageData = {
    recipient: {
      id: recipientId
    },
    message: {
      attachment: {
        type: "image",
        payload: {
          url: "https://www.cryptocompare.com/media/20646/"+cInfo.ticker+".png?width=200"
        }
      }
    }
  }
...
function getTweets(senderID, trend) {
  twitterClient.get('search/tweets', {q: trend}, function(error, body, response) {
    if (error) {
      ...
      var illustratedTweets = body.statuses.filter(t => { return t }).slice(0,3)
      console.log(body.statuses.filter(t => { return t.entities.media }))
      if (illustratedTweets.length > 0) {
        var cards = illustratedTweets.map(t => {
          var card = {
            ...
            image_url: t.entities.media ? t.entities.media[0].media_url : t.user.profile_background_image_url
          }
          return card
        })
